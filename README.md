# Remanga Parser
**Remanga Parser** – это кроссплатформенный скрипт для получения данных с сайта [Remanga](https://remanga.org/) в формате JSON. Он позволяет записать всю информацию о конкретной манге, а также её главах и содержании глав. В скрипт также встроен модуль сборки глав в виде ZIP-архивов.
...
5. В вирутальном окружении указать для выполнения интерпретатором файл `main.py`, передать ему необходимые параметры и запустить.

# Консольные команды
```
build [FILENAME*] [TARGET] [FLAGS] [KEYS]
```
Собирает из описательного файла архивы глав и помещает их в каталог _Output_ внутри директории скрипта. Сборщик поддерживает следующие форматы: [DMP-V1](Examples/DMP-V1.md) и [RN-V2](Examples/RN-V2.md).
